...
import com.wj.store.mapper.AddressMapper;
...
public class AddressServiceImpl implements IAddressService {
    ...
    @Value("${spring.address.max-count}")
    ...
    public void addNewAddress(Integer uid, String username, Address address) {
        ...
        if (count >= maxCount) {
            throw new AddressCountLimitException("用户收货地址超出限额！");
        }

        //对address对象的数据进行补全：省市区
        ...
        if (rows != 1) {
            throw new InsertException("插入用户地址发生未知异常");
        }
    }
    ...
    public Address getByAid(Integer aid,Integer uid) {
        ...
        if(!address.getUid().equals(uid)){
            throw new AccessDeniedException("数据非法异常");
        }
